...
public:
    ...
    bool loadPointCloud()
    {
        ...
        {
            std::string package_path = ros::package::getPath("relocalization");
            if (package_path.empty())
            {
                ROS_ERROR("Could not find package relocalization");
                return false;
            }
    ...
    void cloudSlamCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
    {
        try
        {
            ...
            // 查找从odom到base_footprint的变换
            geometry_msgs::TransformStamped odom_to_base_transform;
            try
            {
                odom_to_base_transform = tf_buffer_.lookupTransform(
                    "base_footprint", "odom", ros::Time(0), ros::Duration(1.0));//获取odom到base_footprint的变换
            }
            catch (tf2::TransformException& ex)
            {
                ROS_WARN_THROTTLE(5.0, "Could not transform from odom to base_footprint: %s", ex.what());
                return;
            }
            ...
            // 使用PCL变换点云从odom坐标系到base_footprint坐标系
            ...
            // 更新SLAM点云（在base_footprint坐标系下，已截取范围）
    ...
    void poseEstimateCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg)
    {
        try
        {
            ...
            //transformation_matrix变换方向是将base_footprint的点变换到map坐标系下
            // 更新map到odom的变换
    ...
    void updateMapToOdomTransform(const Eigen::Matrix4f& map_to_base_transform)
    {
        try
        {
            ...
            base_to_odom_transform = tf_buffer_.lookupTransform(
                "base_footprint", "odom", ros::Time(0), ros::Duration(1.0));
